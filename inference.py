...
def env_state(task_id: str) -> dict:
    r = requests.get(f"{ENV_BASE_URL}/state", params={"task_id": task_id}, timeout=30)
    r.raise_for_status()
    return r.json()
...
def run_episode(task_id: str, episode: int = 1) -> dict:
    ...
    while True:
        ...
        all_budget_ok.append(True)  # simplification; could check price vs budget
...
def compute_grader_score(task_id, rewards, choices, trajectory, categories,
    ...
    if task_id == "task_1_easy":
        ...
        if len(trajectory) >= 2:
            improvements = sum(1 for i in range(1, len(trajectory)) if trajectory[i] >= trajectory[i-1])
